...
class TimeRange(TimePeriod):
    ...
    #TODO: add support for daily and hourly timesteps
    def gen_timesteps_from_tsnumber(self, timesteps_per_year: int) -> Generator[FixedNTimeStep, None, None]:
        """
        ...
        Allows dekadly (36), monthly (12) and yearly data (1).
        """
    ...
    def get_timesteps_from_DOY(self, doy_list: list[int]) -> list[datetime.datetime]:
        return list(self.gen_timesteps_from_DOY(doy_list))

    def get_timesteps_from_issue_hour(self, issue_hours: list[int]) -> list[datetime.datetime]:
        return list(self.gen_timesteps_from_issue_hour(issue_hours))
    
    def gen_timesteps_from_issue_hour(self, issue_hours: list) -> Generator[datetime.datetime, None, None]:
        """
        ...
        while now <= self.end:
            ...
            now = datetime.datetime(day_after.year, day_after.month, day_after.day, issue_hours[0])
